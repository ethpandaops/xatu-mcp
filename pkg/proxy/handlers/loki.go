...
// ServeHTTP handles requests of the form /loki/{instance}/{path...}
func (h *LokiHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Extract instance name from path.
	// Path format: /loki/{instance}/... or /loki/{instance}
	pathParts := strings.SplitN(strings.TrimPrefix(r.URL.Path, "/loki/"), "/", 2)
	...
	// Rewrite path to remove /loki/{instance} prefix.
	remainingPath := "/"
	if len(pathParts) > 1 {
